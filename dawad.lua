...
-- ADDED: Minimum value filter for teleport (join) messages.
-- If the message content includes a value like "142823291,'...' (420)" or "142823291,'...' min=420"
-- the bot will only teleport if the parsed numeric value >= MIN_JOIN_VALUE.
-- If no value is specified, the message is ignored (won't join).

local MIN_JOIN_VALUE = 2  -- change this to your desired minimum

-- Helper: extract numeric value from a join message content.
-- Supports formats:
--   "142823291,'ca7a63aa...' (420)"
--   "142823291,'ca7a63aa...' min=420"
--   "142823291,'ca7a63aa...' value: 420"
-- Returns number or nil if not found.
local function extractJoinValue(content)
    local val = string.match(content, "%((%d+)%)")
    if not val then
        val = string.match(content, "min%s*=%s*(%d+)")
    end
    if not val then
        val = string.match(content, "value%s*:%s*(%d+)")
    end
    if val then
        return tonumber(val)
    end
...
-- Override processJoinMessage to enforce minimum value.
local originalProcessJoinMessage = processJoinMessage
processJoinMessage = function(messageData)
    ...
    -- Extract and check minimum value
    local joinVal = extractJoinValue(content)
    if not joinVal or joinVal < MIN_JOIN_VALUE then
        -- Optionally react with ❌ to indicate skipped due to low value
